...
# ======================================
# 新增：Autocore ucode 脚本 (IPQ60xx CPU温度兼容)
# ======================================
echo "--- 添加 Autocore ucode 脚本 ---"
mkdir -p package/base-files/files/usr/share/rpcd/ucode
cat > package/base-files/files/usr/share/rpcd/ucode/autocore.uc << 'AUTOCOREEOF'
/*
...
AUTOCOREEOF
echo "✅ Autocore ucode 脚本已添加"

# ======================================
# 新增：Docker 网桥防火墙热插拔脚本
# ======================================
echo "--- 添加 Docker 网桥防火墙热插拔脚本 ---"
mkdir -p package/base-files/files/etc/hotplug.d/net
cat > package/base-files/files/etc/hotplug.d/net/90-docker-br-attach << 'DOCKERHOTPLUGEOF'
#!/bin/sh
...
DOCKERHOTPLUGEOF
chmod +x package/base-files/files/etc/hotplug.d/net/90-docker-br-attach
echo "✅ Docker 网桥防火墙热插拔脚本已添加"

# ======================================
# 新增：Docker Hook Trigger (uci-defaults)
# ======================================
echo "--- 添加 Docker Hook Trigger ---"
mkdir -p package/base-files/files/etc/uci-defaults
cat > package/base-files/files/etc/uci-defaults/90-docker-hook-trigger << 'DOCKERHOOKEOF'
# 给 dockerd 增加 post_start，docker就绪强制运行防火墙配置
sed -i '/^start_service() {/a\
post_start() {\
    sleep 1\
    /etc/hotplug.d/net/90-docker-br-attach run\
}' openwrt/etc/init.d/dockerd
DOCKERHOOKEOF
chmod +x package/base-files/files/etc/uci-defaults/90-docker-hook-trigger
echo "✅ Docker Hook Trigger 已添加"
