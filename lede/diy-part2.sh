...
# ======================================
# 0. 清理（★ 去掉了 luci-lib-docker 的删除 ★）
rm -rf package/luci-app-dockerman 2>/dev/null || true
# rm -rf feeds/luci/libs/luci-lib-docker 2>/dev/null || true  # ← 这行删掉
if [ -f .config ]; then
...
# ★★★ 只克隆 kenzok8 的 dockerman 本体 ★★★
echo "--- 克隆 kenzok8 dockerman ---"
...
# luci-lib-docker 由 luci 源提供，确保它被启用
...
# 7. 验证
echo "=== 验证 ==="
[ -d "package/luci-app-dockerman" ] && echo "✅ kenzok8 dockerman 存在" || echo "❌ dockerman 不存在"
grep "^CONFIG_PACKAGE_luci-lib-docker=y" .config && echo "✅ luci-lib-docker 已启用（来自 luci 源）"
grep "^CONFIG_PACKAGE_luci-app-dockerman=y" .config && echo "✅ luci-app-dockerman=y"
